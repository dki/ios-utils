...
for page in pages:
	...
	for offset in cookie_offsets:
		...
		expiry_date_epoch= unpack('<d',cookie.read(8))[0]          #Expiry date is in macOS epoch format
		expiry_date=strftime("%a, %d %b %Y ",gmtime(expiry_date_epoch))[:-1] #[:-1] strips the last space
				
		create_date_epoch=unpack('<d',cookie.read(8))[0]           #Cookies creation time
